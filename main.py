...
TOKEN_ENV_FILE_VAR = "WEBULL_TOKEN_ENV_FILE"
DEFAULT_TOKEN_PATH = os.path.join(os.path.dirname(__file__), "conf", "token.txt")
DEFAULT_ENV_TOKEN_PATH = os.path.join(os.path.dirname(__file__), "conf", "token.env")
...
def resolve_env_token_path() -> str:
    return os.getenv(TOKEN_ENV_FILE_VAR, DEFAULT_ENV_TOKEN_PATH)


def write_token_env_file(token: str | None) -> None:
    if not token:
        return
    env_path = resolve_env_token_path()
    os.makedirs(os.path.dirname(env_path), exist_ok=True)
    with open(env_path, "w", encoding="utf-8") as env_file:
        env_file.write(f"{TOKEN_ENV_VAR}={token}\n")
        ...
        with open(env_path, "r", encoding="utf-8") as env_file:
            for line in env_file:
                ...
                if not stripped or stripped.startswith("#"):
                    continue
                if stripped.startswith(f"{TOKEN_ENV_VAR}="):
